...
def split_up(bins, data):

    splits = [set() for i in range(0, len(bins)+1)]
    for d in data:
        added_it = False
        for count, cut in enumerate(bins):
            if d < cut:
                splits[count].add(d)
                added_it = True
                break
...
output_file("shoeplotave.html", title="example")

p = figure(title = "Prices")
p.xaxis.axis_label = 'price'
p.yaxis.axis_label = 'price'

ave_prices = { k:np.mean(v) for k, v in brands_to_prices.items()}
just_prices = [x for x in ave_prices.values()]

splits = split_up([50.0, 100.0, 250.0, 320.0, 600], just_prices)
